...
import { fetchSearchTagPosts } from "../../services/search";
import useDeletePostMutation from "../../hooks/useDeletePostMutation";
import { notifyError, notifySuccess } from "../../utils/toast";
import {
  SEARCH_STALE_TIME,
  searchTagPostsQueryKey,
} from "./searchQueryKeys";

type TagPostSearchResultsProps = {
  tag: string;
};

const TagPostSearchResults = ({ tag }: TagPostSearchResultsProps) => {
  const {
    data,
    isLoading,
    isError,
    ...
    queryKey: searchTagPostsQueryKey(tag),
    queryFn: ({ pageParam }) => fetchSearchTagPosts({ tag, pageParam }),
    initialPageParam: 0,
...
export default TagPostSearchResults;
